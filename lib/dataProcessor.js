...
DataProcessor.prototype = {
    ...
    getShortStageName: function(longStageName) {
    ...
    getCancerStages: function(neo4jRawJson) {
        ...
        for (var i = 0; i < dataArr.length; i++) {
            var shortStageName = this.getShortStageName(dataArr[i].row[1]);

            // Any shortStageName that is not in the order list, will be ignored.
            if (this.orderedCancerStages.indexOf(shortStageName) !== -1 && uniqueStages.indexOf(shortStageName) === -1) {
                uniqueStages.push(shortStageName);
            }
        }

        // Sort the uniqueCancerFactRelnArr by the item's index in the order array
        var sortedUniqueStages = this.sortByProvidedOrder(uniqueStages, this.orderedCancerStages);
        ...
        sortedUniqueStages.forEach(function(stage) {
            ...
            if (Object.keys(topLevelStages).indexOf(stage) !== -1) {
                for (var i = 0; i < dataArr.length; i++) {
                    var patient = dataArr[i].row[0];
                    var shortStageName = self.getShortStageName(dataArr[i].row[1]);
            ...
            obj.patients.forEach(function(patient) {
                obj.ages.push(self.getPatientEncounterAge(patient.firstEncounterDate, patient.birthday));
            });
    ...
    getPatientCurrentAge: function(birthday) {
        var ageDiffMs = Date.now() - new Date(birthday).getTime();
        var ageDate = new Date(ageDiffMs); // miliseconds from epoch
        return Math.abs(ageDate.getUTCFullYear() - 1970);
    },

    // encounterDateStr is a string, not Date object
    getPatientEncounterAge: function(encounterDateStr, birthday) {
    ...
    getAllPatients: function(neo4jRawJson) {
        //return neo4jRawJson;
        var patientsJson = {};
        var dataArr = neo4jRawJson.results[0].data;
        ...
        for (var i = 0; i < dataArr.length; i++) {
        	var patient = {};
            patient.id = dataArr[i].meta[0].id; // id used later for cancerStages
        	patient.name = dataArr[i].row[0].name;
            patient.firstEncounterAge = this.getPatientEncounterAge(dataArr[i].row[0].firstEncounterDate, dataArr[i].row[0].birthday);
        ...
        for (var j = 0; j < dataArr.length; j++) {
        	...
        	var patientCancerStageShortName = this.getShortStageName(dataArr[j].row[1]);

        	for (var k = 0; k < uniquePatientsArr.length; k++) {
                // Only add stages that are listed in orderedCancerStages
                if (patientId === uniquePatientsArr[k].id && this.orderedCancerStages.indexOf(patientCancerStageShortName) !== -1) {
    ...
    getDiagnosis: function(patientNames, neo4jRawJson) {
        var self = this;
        var diagnosisInfo = {};
        ...
        var dataArr = neo4jRawJson.results[0].data;
        var uniqueDiagnosisArr = [];

        // Build an array of unique diagnosis
        for (var i = 0; i < dataArr.length; i++) {
            var diagnosis = this.normalizeDiagnosis(dataArr[i].row[1]);
        ...
        patientNames.forEach(function(patient) {
            ...
            for (var i = 0; i < dataArr.length; i++) {
                var patientName = dataArr[i].row[0];
                var diagnosis = self.normalizeDiagnosis(dataArr[i].row[1]);
    ...
    normalizeDiagnosis: function(diagnosis) {
        ...
        var mapping = {
            ...
            // Group 2
            "Lobular Breast Carcinoma": "Lobular Carcinoma In Situ",
            "Lobular Breast Carcinoma In Situ": "Lobular Carcinoma In Situ",
            // Group 3
            "Invasive Lobular Breast Carcinoma": "Invasive Lobular Carcinoma",
            // Group 4
            "Invasive Carcinoma": "Invasive Ductal Carcinoma Not Otherwise Specified"
        };
    ...
    getPatientsTumorInfo: function(neo4jRawJson) {
        //console.log(JSON.stringify(neo4jRawJson, null, 4));
        var self = this;

        var patientsTumorInfo = {};
        patientsTumorInfo.data = [];

        var dataArr = neo4jRawJson.results[0].data;
        var uniquePatientsArr = [];

        // Build an array of unique patient names
        for (var i = 0; i < dataArr.length; i++) {
        ...
        uniquePatientsArr.forEach(function(patientName) {
            var patient = {};
            patient.name = patientName;
            patient.tumorSummary = {};

            // Compose the format of neo4jRawJson for each patient
            // so we can get the tumor summary directly by calling `getTumorSummary()`
            var patientNeo4jRawJson = {};
            patientNeo4jRawJson.results = [];
            var resultsObj = {};
            resultsObj.data = [];

            for (var i = 0; i < dataArr.length; i++) {
                if (dataArr[i].row[0] === patientName) {
                    var dataObj = {};
                    dataObj.row = [];
                    ...
                    // Add tthe dataObj to the data array
    ...
    getBiomarkersInfo: function(patientsTumorInfo) {
        var info = {};
        info.patients = patientsTumorInfo.patientList;
        info.biomarkersPool = ["ER", "PR", "HER2"];
        info.data = [];
        ...
        for (var i = 0; i < patientsTumorInfo.data.length; i++) {
            var collatedFactsByCategory = patientsTumorInfo.data[i].tumorSummary.collatedFactsByCategory;
            collatedFactsByCategory.forEach(function(obj) {
                if (info.biomarkersPool.indexOf(obj.category) !== -1) {
                    obj.data.forEach(function(tumor) {
                        tumor.facts.forEach(function(fact) {
                            if (typeof fact.ordinalinterpretation !== "undefined") {
                                biomarkersData[obj.category][fact.ordinalinterpretation .toLowerCase()]++;
                            } else {
                                biomarkersData[obj.category].unknown++;
                            }
                        });
                    });
                }
        ...
        info.biomarkersPool.forEach(function(biomarker) {
            ...
            var totalCount = biomarkersData[biomarker].positive + biomarkersData[biomarker].negative + biomarkersData[biomarker].unknown;
            ...
            if (totalCount > 0) {
                ...
                obj.unknown = parseFloat(biomarkersData[biomarker].unknown / totalCount).toFixed(2);
            }  
    ...
    getPatientInfo: function(neo4jRawJson) {
        var patientInfo = {};

        var dataArr = neo4jRawJson.results[0].data[0].row;

        patientInfo.name = dataArr[0];
        patientInfo.age = this.getPatientCurrentAge(dataArr[1]);
        ...
        patientInfo.menopausal = dataArr[2] ? "Postmenopausal" : "Premenopausal";
    ...
    getFact: function(neo4jRawJson, patientId) {
    	var self = this;
...
//console.log(JSON.stringify(dataArr, null, 4));
        ...
        // There are cases that a Fact has TextMention nodes from different patients
        // So we need to filter out the non-matching patient TextMention nodes
        var filteredUniqueTextProvenances = uniqueTextProvenances.filter(function(obj) {
        	return obj.documentName.split('_')[0].toLowerCase() === patientId.toLowerCase();
        });
        ...
        var docIds = [];
        for (var j = 0; j < filteredUniqueTextProvenances.length; j++) {
            var docId = filteredUniqueTextProvenances[j].documentId;
        ...
        docIds.forEach(function(id) {
            ...
            for (var k = 0; k < filteredUniqueTextProvenances.length; k++) {
...
//console.log(JSON.stringify(groupedTextProvenances, null, 4));
    ...
    prepareTimelineData: function(dataArr) {
        ...
        // Properties
        preparedReports.patientInfo = dataArr[0][5];
        ...
        var reportsGroupedByDateObj = _.groupBy(preparedReports.reportData, function(report) {
            return report.date;
        });

        var reportsGroupedByDateAndTypeObj = {};
        // Then further group by report type
        for (var property in reportsGroupedByDateObj) {
            if (reportsGroupedByDateObj.hasOwnProperty(property)) {
                var arr = reportsGroupedByDateObj[property];
                var reportsGroupedByTypeObj = _.groupBy(arr, function(report) {
                    return report.type;
                });
        ...
        //console.log(JSON.stringify(reportsGroupedByDateAndTypeObj, null, 4));
        ...
        var verticalCountsPerType = {};

        for (var property in reportsGroupedByDateAndTypeObj) {
            if (reportsGroupedByDateAndTypeObj.hasOwnProperty(property)) {
                for (var type in reportsGroupedByDateAndTypeObj[property]) {
                    var arr = reportsGroupedByDateAndTypeObj[property][type];
        ...
        var maxVerticalCountsPerType = {};

        for (var property in verticalCountsPerType) {
