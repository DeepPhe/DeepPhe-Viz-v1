...
class DataProcessor {
    ...
    static getBiomarkers(neo4jRawArr, patientIds) {
        let info = {};
        info.biomarkersPatientsChartData = [];
        info.biomarkersChartData = {};
        info.biomarkersChartData.biomarkersPool = ["has_ER_Status", "has_PR_Status", "has_HER2_Status"];
        info.biomarkersChartData.biomarkerStatus = ['positive', 'negative', 'unknown'];
        info.biomarkersChartData.data = [];
        ...
        info.biomarkersChartData.biomarkersPool.forEach(function(biomarker) {
            let obj = {};
            obj.positive = [];
            obj.negative = [];
            obj.unknown = [];
        ...
        info.biomarkersChartData.biomarkersPool.forEach(function(biomarker) {
            ...
            info.biomarkersChartData.data.push(obj);
        });

        // For biomarkers patients chart
        let patientsWithBiomarkersObj = {};
        patientsWithBiomarkersObj.label = "Patients with biomarkers";
        ...
        patientsWithoutBiomarkersObj.label = "Patients without biomarkers";
        patientsWithoutBiomarkersObj.count = parseFloat(patientsWithoutBiomarkers.length / patientIds.length).toFixed(3);

        info.biomarkersPatientsChartData.push(patientsWithBiomarkersObj);
        info.biomarkersPatientsChartData.push(patientsWithoutBiomarkersObj);
