...
DataProcessor.prototype = {
    ...
    buildTNM: function(collatedFacts, type, tnmClassifications) {
        ...
        // First we need to sort the collatedFacts based on the tnmClassifications order
        // so the order is consistent with the TNM order
        collatedFacts.sort(function(a, b) {
            return tnmClassifications.indexOf(a.category) - tnmClassifications.indexOf(b.category);
        });
        ...
        for (var i = 0; i < collatedFacts.length; i++) {
            if (tnmClassifications.indexOf(collatedFacts[i].category) !== -1) {
                tnmObj.data.push(collatedFacts[i]);
            }
        }
