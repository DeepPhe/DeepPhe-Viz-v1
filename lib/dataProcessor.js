...
DataProcessor.prototype = {
    ...
    getPatientsTumorInfo: function(neo4jRawJson) {
        ...
        patientsTumorInfo.patientsList = [];
        ...
        patientsTumorInfo.patientsList = uniquePatientsArr;
        ...
        uniquePatientsArr.forEach(function(patientName) {
            ...
            patientsTumorInfo.patientsTumorInfo.push(patient);
        });
    ...
    getBiomarkersInfo: function(patientsTumorInfo) {
        var biomarkersInfo = {};
        biomarkersInfo.patients = patientsTumorInfo.patientList;
        biomarkersInfo.biomarkers = [];

        for (var i = 0; i < patientsTumorInfo.patientsTumorInfo.length; i++) {
            var biomarkerObj = {};
            biomarkerObj.patientName = patientsTumorInfo.patientsTumorInfo[i].name;
            biomarkerObj.countsByCategory = [];
            
            var biomarkersPool = ["ER", "PR", "HER2"];

            var arr = patientsTumorInfo.patientsTumorInfo[i].tumorSummary.collatedFactsByCategory;
            arr.forEach(function(obj) {
                if (biomarkersPool.indexOf(obj.category) !== -1) {
                    var categoryObj = {};
                    categoryObj.name = obj.category;
                    categoryObj.status = [];

                    obj.data.forEach(function(tumor) {
                        tumor.facts.forEach(function(fact) {
                            categoryObj.status.push(fact.ordinalinterpretation);
                        });
                    });

                    // Now categoryObj.status is an array of "Positive" or "Nagitive" or "Unknown"

                    biomarkerObj.countsByCategory.push(categoryObj);
                }
            });

            biomarkersInfo.biomarkers.push(biomarkerObj);
        }
    ...
    getCancerStages: function(neo4jRawJson) {
        ...
        sortedUniqueStages.forEach(function(stage) {
            ...
            for (var i = 0; i < dataArr.length; i++) {
                ...
                if ((dataArr[i].row[1] === stage) && (obj.patients.indexOf(patient) === -1)) {
