    ...
    static getDiagnosis(patientIds, neo4jRawArr) {
        ...
        for (let i = 0; i < neo4jRawArr.length; i++) {
            neo4jRawArr[i].diagnosisGroups.forEach(function(diagGrp) {
                let diagnosisGrp = self.normalizeDiagnosis(diagGrp);

                if (uniqueDiagnosisGroupsArr.indexOf(diagnosisGrp) === -1) {
                    uniqueDiagnosisGroupsArr.push(diagnosisGrp);
                } 
            });
        }

        diagnosisInfo.diagnosisGroups = uniqueDiagnosisGroupsArr;
        ...
        patientIds.forEach(function(pid) {
            ...
            for (let i = 0; i < neo4jRawArr.length; i++) {
                neo4jRawArr[i].diagnosisGroups.forEach(function(diagGrp) {
                    ...
                    if (neo4jRawArr[i].patientId === pid && obj.diagnosisGroups.indexOf(diagnosisGrp) === -1) {
                        obj.diagnosisGroups.push(diagnosisGrp);
                    } 
                });
            }
