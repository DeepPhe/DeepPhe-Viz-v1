...
DataProcessor.prototype = {
    ...
    getTumorSummary: function(neo4jRawJson) {
        ...
        summary.tumors = [];
        summary.commonCategories = [];

        var dataArr = neo4jRawJson.results[0].data;

        // Build an arry of unique tumors (id and name)
        for (var i = 0; i < dataArr.length; i++) {
            if (summary.tumors.indexOf(dataArr[i].row[0]) ===-1 && dataArr[i].row[1]=== 'hasTumorType') {
                var tumorObj = {};
        ...
        for (var k = 0; k < sortedFactRelationships.length; k++) {
            ...
            obj.category = this.toNonCamelCase(sortedFactRelationships[k]);
            obj.data = [];

            // Add collacted facts array for each tumor
            for (var n = 0; n < summary.tumors.length; n++) {
                var factsObj = this.getTumorFactsByRelationship(summary.tumors[n].id, obj.category, dataArr);
                obj.data.push(factsObj);
            }

            summary.commonCategories.push(obj);
        }

        return summary;
    },

    getTumorFactsByRelationship: function(tumorId, category, dataArr) {
        var factsObj = {};
        ...
        for (var i = 0; i < dataArr.length; i++) {
            ...
            var targetTumorFactRel = this.toNonCamelCase(dataArr[i].row[1]);
...
//console.log(JSON.stringify(dataArr, null, 4));
    ...
    getReport: function(neo4jRawJson, factMentionTermsArr) {
        //console.log(JSON.stringify(neo4jRawJson, null, 4));
        var dataArr = neo4jRawJson.results[0].data;
        ...
        for (var i = 0; i < dataArr.length; i++) {
            ...
            if (dataArr[i].row[1] !== null) {
                ...
                term.count = dataArr[i].row[2];
                // Add to the array
                report.mentionedTerms.push(term);
            }
        }

        // factMentionTermsArr can be empty array []
        if (factMentionTermsArr.length > 0) {
            report.mentionedTerms.forEach(function(obj) {
                if (factMentionTermsArr.indexOf(obj.text) !== -1) {
                    // Add a new property to indicate this term is the related fact mention and highlight
                    obj.factMention = true;
                }
            });
        }
