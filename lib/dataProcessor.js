...
DataProcessor.prototype = {
    ...
    getCohortData: function(neo4jRawJson) {
        ...
        let sortedUniqueStages = this.sortByProvidedOrder(uniqueStages, this.orderedCancerStages);
    ...
    getAllPatients: function(neo4jRawJson) {
        ...
        for (let j = 0; j < dataArr.length; j++) {
        	let patientId = dataArr[j].meta[0].id;
    ...
    getBiomarkersInfo: function(patientsTumorInfo) {
        ...
        for (let i = 0; i < patientsTumorInfo.data.length; i++) {
            let collatedFactsByCategory = patientsTumorInfo.data[i].tumorSummary.collatedFactsByCategory;
    ...
    getCancerSummary: function(neo4jRawJson) {
        ...
        // Sort the uniqueCancerFactRelnArr by the item's index in the order array
        let sortedUniqueCancerFactRelnArr = this.sortByProvidedOrder(uniqueCancerFactRelnArr, order);
        ...
        let unspecifiedTNM = this.buildTNM(allCollatedFacts, "Unspecified", tnmClassifications.unspecified);
        let clinicalTNM = this.buildTNM(allCollatedFacts, "Clinical", tnmClassifications.clinical);
        let pathologicTNM = this.buildTNM(allCollatedFacts, "Pathologic", tnmClassifications.pathologic);
    ...
    sortByTumorType: function(array, orderArr) {
        ...
        // Sort the original array by the item's index in the orderArr
        let sortedArray = _.sortBy(array, function(item){ 
    ...
    formatTumorType: function(type) {
        ...
        let str = type.replace('_', '').replace(/([A-Z])/g, " $1" );
