...
class DataProcessor {
    ...
    static getBiomarkers(neo4jRawArr) {
        ...
        info.biomarkersPool = ["has_ER_Status", "has_PR_Status", "has_HER2_Status"];
        ...
        neo4jRawArr.forEach(function(obj) {
        	...
        	if (typeof obj.tumorFact.valueText !== "undefined") {
                ...
                biomarkersData[obj.tumorFactRelation][status]++;
        	} 
        });
        ...
        info.biomarkersPool.forEach(function(biomarker) {
            ...
            if (totalCount > 0) {
                obj.positive = parseFloat(biomarkersData[biomarker].positive / totalCount).toFixed(3);
                obj.negative = parseFloat(biomarkersData[biomarker].negative / totalCount).toFixed(3);
                obj.unknown = parseFloat(1 - obj.positive - obj.negative).toFixed(3);
            }  
    ...
    static getCancerAndTumorSummary(neo4jRawArr) {
        let self = this;
console.log('cancers: ' + JSON.stringify(neo4jRawArr, null, 4));
        let cancers = [];
...
console.log('allCollatedCancerFacts: ' + JSON.stringify(allCollatedCancerFacts, null, 4));

console.log('clinicalTNM: ' + JSON.stringify(clinicalTNM, null, 4));

console.log('pathologicTNM: ' + JSON.stringify(pathologicTNM, null, 4));
    ...
    // E.g., convert "has_Clinical_T" into "Clinical T"
    // Exception: "hasBIRADSCategory" and "hasKi67Status"
    static formatCategoryName(text) {
        if (text === 'hasKi67Status') {
            return 'Ki-67';
        } else if (text === 'hasBIRADSCategory') {
            return 'BIRADS'; // Also omit "Category"
        } else if (text.startsWith("has_")) {
            // Remove 'has_' from the beginning then replace underscore with space
            let str = text.substring(4).replace(/_/g, " " );
            // Trim out the beginning space if any
            return str.trim();
