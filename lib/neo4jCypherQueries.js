...
var neo4jCypherQueries = {
	...
	getPatients: function(stage) {
		var query = "";

        // Find and return all nodes with Patient label
		// p is the node variable name, Patient is the node label
		if (stage.length > 0) {
            query = "MATCH (p:Patient)-->(cancer:Cancer)-[cancerFactReln:hasCancerStage]->(fact:Fact) " +
		            "WHERE fact.prettyName = '" + stage + "' " +
    ...
    getCancerStages: function() {
		var query = "MATCH (cancer:Cancer)-[r:hasCancerStage]->(f:Fact) " +
					"RETURN DISTINCT f.prettyName";
		return {"statement": query};
	},
    ...
    getReport: function(reportId) {
		var query = "MATCH (r:Report {id:'" + reportId + "'}) " +
					...
					"RETURN r.text, n.text, count(n.text)";
		return {"statement": query};
	},
    ...
    getTumorSummary: function(patientName, cancerId) {
        var query = "MATCH (patient:Patient)-->(cancer:Cancer)-[cancerTumorReln:hasTumor]->(tumor:Tumor)-[tumorFactReln]->(fact:Fact) " +
                    ...
                    "OPTIONAL MATCH (patient:Patient)-->(cancer:Cancer)-[cancerTumorReln:hasTumor]->(tumor:Tumor)-[tumorFactReln]->(fact:Fact)-[rel]->(f:Fact) " +
