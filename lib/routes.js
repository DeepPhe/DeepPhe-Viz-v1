            ...
            httpRequest({
                ...
                if ( ! error) {
                    ...
                    var data = {
                        ...
                        commonCategories: summary.commonCategories
                    };
    ...
    // Single report URI route, called by client ajax, no view template rendering
    {
        ...
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                json: {
                    'statements': [neo4jCypherQueries.getMentionedTexts(reportId)]
                }
            }, function (error, response, body) {
                if ( ! error) {
                    //console.log('response: ' + JSON.stringify(response, null, 4));
                    var mentionedTexts = dataProcessor.getMentionedTexts(body);
                    //console.log('response: ' + JSON.stringify(mentionedTexts, null, 4));

                    // Render mentionedTexts.html
                    var data = {
                        mentionedTexts: mentionedTexts
                    };

                    // Specify to use the empty layout instead of the default layout
                    // This way we can send the rendered content as response directly
                    reply.view('mentionedTexts', data, {layout: 'empty'});
                } else {
                    console.log('Failed to make the neo4j rest api call: getMentionedTexts()');
                    console.error(error);
                }
            });
        }
            ...
            httpRequest({
                ...
                if ( ! error) {
                    ...
                    var data = {
                        ...
                        groupedTextMentions: factJson.groupedTextMentions
                    };
