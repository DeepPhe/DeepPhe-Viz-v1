...
const routes = [
        ...
        handler: function (request, h) {
            ...
            let promise = session.run(neo4jFunctions.getPatientsTumorInfo(patientIds))
                .then(function(neo4jResult) {
                    ...
                    let neo4jRawArr = neo4jResult.records[0].get('patientsTumorInfo');

                    // Convert the result into desired json data structure
                    let patientsTumorInfo = dataProcessor.getPatientsTumorInfo(neo4jRawArr);
        ...
        handler: function (request, h) {
            ...
            let promise = session.run(neo4jFunctions.getReport(reportId))
                .then(function(neo4jResult) {
                    ...
                    // The neo4j function returns the report text and all textMentions directly
                    return neo4jResult.records[0].get('report');
                })
                .catch(function(error) {
    ...
    {
        method: 'GET',
        path: '/' + apiBaseUriPath + '/tumorinfo/{patientIds}', 
        // For API documentation with Swagger
        options: {
	        description: 'Patients tumor information', 
	        notes: 'Returns the grouped tumor information in JSON for a list of given patient IDs (seperated using the plus sign "+")',
	        tags: ['api'],
	        validate: {
	            params: {
