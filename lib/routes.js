...
var routes = [
        ...
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                json: {
                    ...
                    // pass in empty array to get all patients without any filters
        ...
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                if ( ! error) {
                    ...
                    // Render cancerStages.html
                    var data = {
                        stages: stagesObj.stages
                    };
                    ...
                    reply.view('cancerStages', data, {layout: 'empty'});
                } else {
    ...
    {
        method: 'GET',
        path:'/cohort/{stage?}', // stage is optional
        handler: function (request, reply) {
            // Pass in stage as an array since it may contain multiple stages
            var stage = request.params.stage ? decodeURIComponent(request.params.stage) : '';
            ...
            httpRequest({
                ...
                json: {
                    // Neo4j Transactional Cypher HTTP request body
                    // "statements" is an array of statements, we only have one statement/query per request
                    'statements': [neo4jCypherQueries.getPatients(stage)]
                }
            }, function (error, response, body) {
                if ( ! error) {
                    //console.log('response: ' + JSON.stringify(response, null, 4));
                    ...
                    request.server.render('cohort', data, {layout: 'empty'}, (err, rendered, config) => {
                        // Because we get the rendered view through this callback, the final reply needs to be inside the callback
                        reply(rendered);
                    });
                } else {
                    console.log('Failed to make the neo4j rest api call: getCohort()');
                    console.error(error);
                }
            });
        }
