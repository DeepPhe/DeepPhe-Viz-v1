...
var routes = [
    ...
    // generate DOB to each patient node in Neo4j
    {
        method: 'GET',
        path:'/generateBirthday', 
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                json: {
                    ...
                    'statements': [neo4jCypherQueries.getAllPatients()]
                }
            }, function (error, response, body) {
                if ( ! error) {
                    //console.log('response: ' + JSON.stringify(response, null, 4));
                    
                    var patientsJson = dataProcessor.generatePatientBirthdays(body);
    ...
    // Add DOB to each patient node in Neo4j
    {
        method: 'GET',
        path:'/addBirthday/{patientName}/{birthday}', 
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                json: {
                    // Neo4j Transactional Cypher HTTP request body
                    // "statements" is an array of statements, we only have one statement/query per request
                    'statements': [neo4jCypherQueries.addBirthday(patientName, birthday)]
                }
            }, function (error, response, body) {
                if ( ! error) {
                    //console.log('response: ' + JSON.stringify(response, null, 4));
                    ...
                    console.log('Failed to make the neo4j rest api call: addBirthday()');
                    console.error(error);
                }
            });
        }
    ...
    // Cohort Analysis URI route
    {
        ...
        handler: function (request, reply) {
            ...
            reply.view('cohortAnalysis', data);
        }
    },
        ...
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                json: {
                    // Neo4j Transactional Cypher HTTP request body
                    // "statements" is an array of statements, we only have one statement/query per request
                    'statements': [neo4jCypherQueries.getCancerStages()]
                }
            }, function (error, response, body) {
                if ( ! error) {
                    //console.log('response: ' + JSON.stringify(response, null, 4));
                    ...
                    var stagesObj = dataProcessor.getCancerStages(body);

                    reply(stagesObj);
                } else {
                    console.log('Failed to make the neo4j rest api call: getCancerStages()');
                    console.error(error);
                }
            });
        }
        ...
        handler: function (request, reply) {
            ...
            httpRequest({
                ...
                if ( ! error) {
                    ...
                    // Convert the body into desired json data structure
                    var patientsJson = dataProcessor.getPatients(body, stage);

                    // Data to render patients.html
                    var data = {
                        baseUri: request.server.info.uri, // Get the base uri via server.info 
                        numOfPatients: patientsJson.patients.length,
                        targetStage: (stage === null) ? "All Stages" : stage,
                    ...
                    request.server.render('patients', data, {layout: 'empty'}, (err, rendered, config) => {
