...
const routes = [
    ...
    // Tumor chart URI route
    {
        ...
        handler: function (request, h) {
            ...
            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getPatientsTumorInfo(patientNames, cancer.id)))
                .then(function(body) {
                    // Convert the body into desired json data structure
    ...
    // Diagnosis chart URI route
    {
        ...
        handler: function (request, h) {
            ...
            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getDiagnosis(patientNames, cancer.id)))
                .then(function(body) {
                    // Convert the body into desired json data structure
    ...
    // Tumors summary URI route, called by client ajax
    {
        ...
        handler: function (request, h) {
            let patientName = request.params.patientName;
            let cancerId = request.params.cancerId;

            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getTumorSummary(patientName, cancerId)))
                .then(function(body) {
                    // Convert the body into desired json data structure
