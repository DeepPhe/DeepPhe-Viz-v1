...
const routes = [
    ...
    // All patients from all stages URI route
    {
        ...
        path:'/cohortData', 
        handler: function (request, h) {
//            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getCohortData()))
            var session = driver.session();
            let promise = session.readTransaction(tx => neo4jBoltFunctions.getCohortData(tx))
                .then(function(body) {
                  session.close();
...
//            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getPatientsTumorInfo(patientIds)))
            var session = driver.session();
...
//            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getDiagnosis(patientIds)))
            var session = driver.session();
            let promise = session.readTransaction(tx => neo4jBoltFunctions.getDiagnosis(tx,patientIds))
                .then(function(body) {
                  session.close();
                    // Convert the body into desired json data structure
                    return dataProcessor.getDiagnosis(patientIds, body);
                })
                .catch(function(error) {
                    session.close();
...
//            let promise = httpRequestPromise(neo4jRestCall(neo4jCypherQueries.getTimelineData(patientName)))
            var session = driver.session();
